...
SR_m3_data <- SR_SRout_ccc %>%
  ...
  mutate(anti_pos = ifelse(num_anti == "0", 0, 1))
table(SR_m3_data$anti_pos, useNA = "ifany")
...
m1_plots_abc <- patchwork::wrap_plots(
  ...
  #boxplot
  ggplot(SR_m12_data, aes(x=factor(anti_pos, 
                                   levels = c("1","0"), 
                                   labels = c("Positive", 
                                              "Negative")), 
                          y = pred_prob_m1)) +
    ...
    geom_boxplot(width = .15, 
                 outlier.size = 1) +
    xlab("Antibody status")+
    ylab("Predicted probability of \n Antibody Positive vs Negative") +
    theme_bw(),
  #calibration plot
...
m3_plots_abc <- patchwork::wrap_plots(
  ...
  #boxplot
  ggplot(SR_m3_data, aes(x=factor(anti_pos, 
                                  levels = c("1","0"), 
                                  labels = c("Positive", 
                                             "Negative")), 
                         y = pred_prob_m3)) +
...
####PDF
# pdf("figures/Supp_Figure19.pdf", height = 15, width = 16)
...
####PDF
pdf("figures/Supp_Figure19.pdf", height = 5, width = 16)
model_display_item <- patchwork::wrap_plots(
  m1_plots_abc
) + patchwork::plot_annotation(tag_levels = list(c("", "A", "B", "C")))
print(model_display_item)
