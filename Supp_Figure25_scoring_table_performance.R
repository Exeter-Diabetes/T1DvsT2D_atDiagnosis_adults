...
table(SR_mD_data$eth, useNA = "ifany")
table(SR_m12_data$ethnicity, useNA = "ifany")

SR_mD_data <- SR_mD_data %>%
  select(Study_ID, SRoutcome, robust_outcome, m1_pp, agedx, bmi, hba1c, sex,
         osmo, auto, weightloss, dka, eth,
         pardm, pos_anti) %>%
...
SR_50_mD_data <- SR_50_mD_data %>%
  select(Study_ID, SRoutcome, robust_outcome, m1_pp, agedx, bmi, hba1c, sex,
         osmo, auto, weightloss, dka,eth, pardm, pos_anti) %>%
  mutate(ScoreD = select(., agedx:pardm) %>% rowSums(na.rm = TRUE))
...
model_continuous(varlist,
                 ...
                 outcome = "SRoutcome",
                 saving_name = "tables/Supp_Table25_Prime",
                 complete_case = TRUE,
                 plots = TRUE,
                 manual_plotting = TRUE,
...
#Both plots as PDF ---------------------------------------------------------------
pdf("figures/Supp_Figure25_score_plots.pdf", height = 10, width = 16)
...
dev.off()
ggsave("figures/Supp_Figure25_score_plots.jpeg", model_display_item, height = 10, width = 16)
