...
SR_ins_ccc <- SR_SRout_ccc %>%
  filter(!is.na(insulinRequire))
SR_ins_ccc$insulinRequire <- as.numeric(SR_ins_ccc$insulinRequire)
#Model12
SR_ins_m12_data <- SR_ins_ccc %>%
  drop_na(all_of(all_vars_12))
#Model3
SR_ins_m3_data <-SR_ins_ccc %>%
  drop_na(all_of(all_vars_3))

####Model 1 ROW --------------------------------------------------------------------------
####Get large amount of model info automated
model_info(model = m1, 
           test_data = SR_ins_m12_data, 
           outcome = "insulinRequire",
           saving_name = "03_05_sm1_ins", 
...
####Get large amount of model info automated
model_info(model = m3, 
           test_data = SR_ins_m3_data, 
           outcome = "insulinRequire",
           saving_name = "03_05_sm3_ins", 
           manual_plotting = TRUE, 
           manual_plot_name = "m3")
...
11111#
  ...
  223344
"
m12_n <- nrow(SR_ins_m12_data)
m3_n <- nrow(SR_ins_m3_data)
...
m1_plots_abc <- patchwork::wrap_plots(
  ...
  #boxplot
  ggplot(SR_ins_m12_data, aes(x=factor(insulinRequire, 
                                       levels = c("1","0"), 
                                       labels = c("Progressed ", 
                                                  "Did not progress")), 
                              y = pred_prob_m1)) +
...
m3_plots_abc <- patchwork::wrap_plots(
  ...
  #boxplot
  ggplot(SR_ins_m3_data, aes(x=factor(insulinRequire, 
                                      levels = c("1","0"), 
                                      labels = c("Progressed", 
                                                 "Did not progress")), 
                             y = pred_prob_m3)) +
...
####PDF
pdf("figures/Supp_Figure21.pdf", height = 15, width = 16)
