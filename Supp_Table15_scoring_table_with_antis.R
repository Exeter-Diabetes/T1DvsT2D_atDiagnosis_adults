...
SR_mD_data <- SR_m12_data %>%
  select(Study_ID, SRoutcome, robust_outcome, AgeatDiagnosis, bmi_model,
         ...
         famhisnoninsdiab, num_anti, ethnicity) %>%
  mutate(
...
SR_mD_data <- SR_mD_data %>%
  select(Study_ID, SRoutcome, robust_outcome, m1_pp, agedx, bmi, hba1c, sex,
         osmo, auto, weightloss, dka, eth, pardm, pos_anti, anti_cat) %>%
...
##Model 2  --------------------------------------------------------------------------
# m2 <- glm(SRoutcome ~ relevel(factor(SR_score_cat), ref = "<5")*relevel(factor(anti_cat), ref = "0"),
#           data = SR_mD_data,
#           family = binomial)
# summary(m2)
#
# decimals <- 3
#
# coef_t <- as.data.frame(t(round(m2$coefficient, decimals)))
# odds_ratio_t <- as.data.frame(t(round(exp(m2$coefficient),decimals)))
#
#
# #coefficients for coefficients table
# ##log-odds/estimate/beta
# estimate <- as.data.frame(round(m2$coefficient, decimals))
# ##log-odds/estimate confidence intervals
# conf_int <- as.data.frame(confint(m2))
# ##odds-ratio
# odds_ratio <- as.data.frame(round(exp(cbind(OR = coef(m2),
#                                             confint(m2))),
#                                   decimals)) %>%
#   rename(
#     OR_lower = `2.5 %`,
#     OR_upper = `97.5 %`
#   )
# p_values <- as.data.frame(summary(m2)$coefficients[, 'Pr(>|z|)']) %>%
#   rename(p_value = `summary(m2)$coefficients[, "Pr(>|z|)"]`)
#
#
...
# coefs <- coefs %>%
#   rename(
#     estimate = `round(m2$coefficient, decimals)`,
#     lower = `2.5 %`,
#     upper = `97.5 %`
#   )
#
...
#          `Antis B coefficient` = coef,
#          `Antis OR` = OR,
#          `Antis p value` = p_value,
#          `Antis Score` = SCORE
#   )
# #coef_table <- left_join(coef_table_m2, coef_table_m1)
# write_xlsx(coef_table_m2,"tables/scoring_tables_antis.xlsx")

###Model 2  --------------------------------------------------------------------------
m2 <- glm(SRoutcome ~ relevel(agediag, ref = ">=45") +
...
coef_table <- left_join(coef_table_m2, coef_table_m1)
write_xlsx(coef_table,"tables/Supp_Table15_antis_scoring_table.xlsx")
