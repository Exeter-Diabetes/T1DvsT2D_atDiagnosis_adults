...
#This provides sense checks to 01_02
...
#counts per category
table(SR_50_SRout$eat5h, useNA = "ifany")
...
#counts per category
table(SR_50_ro$eat5h, useNA = "ifany")
...
table(SR_50_SRout$glu_cp, useNA = "ifany")
...
table_01_12_I_2_A_details <- SR_50_SRout %>%
  filter(glu_cp == "glu < 4 & Cpep < 600") %>%
  dplyr::select(Study_ID, SRoutcome, define, venous_glucose_v4, c_peptide_v4, UCPCR_v4)
write_xlsx(table_01_12_I_2_A_details,"tables/table_01_12_I_2_A_details.xlsx")
...
#look at category counts
table(SR_50_ro$glu_cp, useNA = "ifany")
#compare missing categry with missing glucose
SR_50_ro %>%
  filter(is.na(venous_glucose_v4)) %>%
  count()
#look at those with low glucose and low c-peptide more closely
table_01_12_I_2_B_details <- SR_50_ro %>%
  filter(glu_cp == "glu < 4 & Cpep < 600") %>%
  dplyr::select(Study_ID, SRoutcome, define, venous_glucose_v4, c_peptide_v4, UCPCR_v4)
write_xlsx(table_01_12_I_2_B_details,"tables/table_01_12_I_2_B_details.xlsx")
...
write_xlsx(table_01_12_I_3_A_details,"tables/table_01_12_I_3_A_details.xlsx")
...
table_01_12_I_3_B_details <- SR_50_ro %>%
  filter(as.numeric(urine_days_in_transit_v4) > 7) %>%
  dplyr::select(Study_ID, urine_days_in_transit_v4, SRoutcome, define, UCPCR_v4)
write_xlsx(table_01_12_I_3_B_details,"tables/table_01_12_I_3_B_details.xlsx")
...
SR_50_SRout$autoimmune <- as.numeric(SR_50_SRout$autoimmune)
SR_50_SRout$osmotic <- as.numeric(SR_50_SRout$osmotic)
SR_50_SRout$DKA <- as.numeric(SR_50_SRout$DKA)
SR_50_SRout$num_anti <- factor(SR_50_SRout$num_anti)
SR_50_SRout$LDL_v1 <- as.numeric(SR_50_SRout$LDL_v1)
SR_50_SRout$Urea_v1 <- as.numeric(SR_50_SRout$Urea_v1)
SR_50_SRout$Haemoglobin_v1 <- as.numeric(SR_50_SRout$Haemoglobin_v1)
...
#save as
table_01_12_II_1_characteristics_medIQR <- as.data.frame(summaryTable_GROUP_missing)
write_xlsx(table_01_12_II_1_characteristics_medIQR,"tables/table_01_12_II_1_characteristics_medIQR.xlsx")


#Figures for above
distri_plot(dataset = SR_50_SRout, vars = varlist, pdf_name = "plots_01_12_II_1_bydef3", group = "def3")




#ant
table(SR_50_SRout$num_anti)
table(SR_50_SRout$AntibodyStatus)
#visualising bmi distribution

SR_50_SRout$num_anti <- as.character(SR_50_SRout$num_anti)

ggplot(SR_50_SRout, aes(num_anti)) +
  geom_bar() +
  facet_wrap(vars(def3))

SR_50_SRout$AntibodyStatus <- as.character(SR_50_SRout$AntibodyStatus)

ggplot(SR_50_SRout, aes(AntibodyStatus)) +
  geom_bar() +
  facet_wrap(vars(def3))
...
table_01_12_II_2_c_details <- d_01_12_II_2 %>%
  filter(ucpcr_cat == ">=1.1") %>%
  dplyr::select(clinical_diagnosis_v1, UCPCR_v3, UCPCR_v4, Other_type_of_diabetes_v1)
write_xlsx(table_01_12_II_2_c_details,"tables/table_01_12_II_2_c_details.xlsx")
...
write_xlsx(table_01_12_II_3_characteristics_medIQR,"tables/table_01_12_II_3_characteristics_medIQR.xlsx")
...
table_01_12_II_3_a_details <- d_01_12_II_3 %>%
  filter(ins3 == "insulin v3" | ins3 == "insulin v4" | ins3 == "insulin v4 up") %>%
  filter(ucpcr_cat == ">1.1" | ucpcr_cat == "0.6 - <1.1") %>%
  dplyr:: select(SRoutcome, define, ucpcr_cat)
write_xlsx(table_01_12_II_3_a_details,"tables/table_01_12_II_3_a_details.xlsx")
#
...
# 3) remove invalid -------------------------------------------------------------------------------------
## A) For SRoutcome -------------------------------------------------------------------------------------
SR_50_SRout_cc <- SR_50_SRout
save(SR_50_SRout_cc, file = "data/SR_50_SRout_cc_2_2025.RData")

## B) For robust_outcome -------------------------------------------------------------------------------------
SR_50_ro_cc <- SR_50_ro
save(SR_50_ro_cc, file = "data/SR_50_ro_cc_2_2025.RData")
