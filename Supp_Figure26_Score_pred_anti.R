...
SR_mD_data <- SR_m12_data %>%
  select(Study_ID, SRoutcome, robust_outcome, AgeatDiagnosis, bmi_model,
         ...
         famhisnoninsdiab, num_anti, ethnicity, anti_pos) %>%
  mutate(
...
SR_mD_data <- SR_mD_data %>%
  select(Study_ID, SRoutcome, robust_outcome, m1_pp, agedx, bmi, hba1c, sex,
         osmo, auto, weightloss, dka,eth, pardm, pos_anti, anti_pos) %>%
...
table(SR_50_mD_data$ScoreD, useNA = "ifany")
varlist = c("ScoreD")
model_continuous(varlist,
...
"
##Calibration prep
dec_SR_nm1 <- data.frame(y = as.numeric(SR_mD_data$anti_pos),
...
score_SR_plots_abc <- patchwork::wrap_plots(
  ...
  #boxplot
  ggplot(SR_mD_data, aes(x=factor(anti_pos,
                                  levels = c("1","0"),
                                  labels = c("Positive", "Negative")),
                         y = ScoreD)) +
    geom_violin(aes(fill = factor(anti_pos,
                                  levels = c("1","0"),
                                  labels = c("Positive", "Negative")), alpha = 0.2)) +
    geom_boxplot(aes(fill = factor(anti_pos,
                                   levels = c("1","0"),
                                   labels = c("Positive", "Negative"))),width = .15,
                 outlier.size = 1) +
...
##Calibration prep
dec_PRIME_nm1 <- data.frame(y = as.numeric(SR_50_mD_data$anti_pos),
...
score_prime_plots_abc <- patchwork::wrap_plots(
  ...
  #boxplot
  ggplot(SR_50_mD_data, aes(x=factor(anti_pos,
                                     levels = c("1","0"),
                                     labels = c("Positive", "Negative")),
                            y = ScoreD)) +
    geom_violin(aes(fill = factor(anti_pos,
...
#Both plots as PDF ---------------------------------------------------------------
pdf("figures/Supp_Figure26_score_plots.pdf", height = 10, width = 16)
...
dev.off()
ggsave("figures/Supp_Figure26_score_plots.jpeg", model_display_item, height = 10, width = 16)
