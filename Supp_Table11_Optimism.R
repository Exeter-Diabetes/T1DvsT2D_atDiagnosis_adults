...
SR_12_dd <- SR_m12_data %>%
  dplyr::select(all_of(all_vars_12), "Eth_4cat", "SRoutcome") %>%
  mutate(Eth_4cat = factor(Eth_4cat, levels = c("White", "Black", "South Asian", "Other/Mixed"))) %>%
  as.data.frame()
...
###Model 3 ------------------------------------------------------------------------
SR_3_dd <- SR_m3_data %>%
  dplyr::select(all_of(all_vars_3), "Eth_4cat", "SRoutcome") %>%
  mutate(Eth_4cat = factor(Eth_4cat, levels = c("White", "Black", "South Asian", "Other/Mixed"))) %>%
  as.data.frame()
...
#   as.data.frame()
# dd3 <- datadist(SR_3_dd); options(datadist = 'dd')
m4_rms <- lrm(SRoutcome ~ scale(AgeatDiagnosis) +
                scale(bmi_model) +
                scale(HbA1c_at_diagnosis_v1) +
                #scale(c_peptide_v1) +
                Gender_v1 +
...
m1_optimism <- rbind(m1_optimism,
                     m1_optimism_add)
write_xlsx(m1_optimism,"tables/Supp_Table11_m1.xlsx")
###Model 2 -------------------------------------------------------------------------------
#### RMS: VALIDATE -------------------------------------------------------------------------
...
m2_optimism <- rbind(m2_optimism,
                     m2_optimism_add)
write_xlsx(m2_optimism,"tables/Supp_Table11_m2.xlsx")
###Model 3 -------------------------------------------------------------------------------
#### RMS: VALIDATE -------------------------------------------------------------------------
...
write_xlsx(m3_optimism,"tables/Supp_Table11_m3.xlsx")

###Model 4 -------------------------------------------------------------------------------
#### RMS: VALIDATE -------------------------------------------------------------------------
m4_optimism_rms <- rms::validate(m4_rms,
                                 B = 1000,
                                 maxit = 20)
m4_optimism <- unclass(m4_optimism_rms)
m4_optimism <- data.frame(`Performance metric` = row.names(m4_optimism),
                          m4_optimism)
colnames(m4_optimism) <- c("Performance metric",
                           "Full",
                           "Bootstrap:training",
                           "Bootstrap:test",
                           "Optimism",
...
original_predictions4 <- predict(m4,
                                 newdata = SR_m3_data,
                                 type = "response") %>%
  cbind(id = SR_m3_data$id)

full_roc_model <- roc(SR_m3_data$SRoutcome,
                      original_predictions4[,1],
                      plot = FALSE,
                      ci = TRUE)
full_calinlarge <- mean(original_predictions4[,1],
                        na.rm = TRUE) - (sum(SR_m3_data$SRoutcome)/nrow(SR_m3_data))
#Extract valuable summary information from ROC object
...
m4_optimism_add <- data.frame(
  ...
  rbind(m4_optimism_rocs)
colnames(m4_optimism_add) <- c("Performance metric",
                               "Full",
                               "Bootstrap:training",
                               "Bootstrap:test",
                               "Optimism",
...
m4_optimism <- rbind(m4_optimism, m4_optimism_add)
write_xlsx(m4_optimism,"tables/Supp_Table11_m4.xlsx")
