...
SR_SRout_ccc<- SR_SRout_ccc %>%
  mutate(bmi_model = ifelse(is.na(bmi_diag),
         ...
         Eth_4cat = factor(ifelse(Eth_5cat %in% c("Other", "Mixed"), "Other/Mixed", Eth_5cat))
  )
...
# Function to produce roc_curves_mX, pred_prob_mX, and cal_prep_mX
...
# model_info(model = m1,
#            test_data = SR_Black_12,
#            outcome = "SRoutcome",
#            saving_name = "03_05_sm1_Black",
...
# ####Get large amount of model info automated
# model_info(model = m3,
#            test_data = SR_Black_3,
#            outcome = "SRoutcome",
#            saving_name = "03_05_sm3_Black",
#            manual_plotting = TRUE,
#            manual_plot_name = "m3")
# ####Model 4 ROW --------------------------------------------------------------------------
# ####Get large amount of model info automated
...
11111#
  ...
  223344
"
m12_n <- nrow(SR_Black_12)
m3_n <- nrow(SR_Black_3)
n12_T1D <- SR_Black_12 %>%
  filter(SRoutcome == 1) %>%
  nrow()
n3_T1D <- SR_Black_3 %>%
  filter(SRoutcome == 1) %>%
  nrow()
SR_Black_12 <- SR_Black_12 %>%
  mutate(SRoutcome = as.character(SRoutcome))
SR_Black_3 <- SR_Black_3 %>%
  mutate(SRoutcome = as.character(SRoutcome))
...
dat_text_m1 <- dat_text_m1 %>%
  mutate(auc_full = "AUCROC: 0.86 (0.65;1.00)")
dat_text_m2 <- dat_text_m2 %>%
  mutate(auc_full = "AUCROC: 0.95 (0.87;1.00)")
dat_text_m3 <- dat_text_m3 %>%
  mutate(auc_full = "AUCROC: 0.96 (0.88;1.00)")
dat_text_m4 <- dat_text_m4 %>%
  mutate(auc_full = "AUCROC: 0.89 (0.69;1.00)")
#### Model1 row output
m1_plots_abc <- patchwork::wrap_plots(
  ...
  #boxplot
  ggplot(SR_Black_12, aes(x=factor(SRoutcome,
                                       levels = c("1","0"),
...
m3_plots_abc <- patchwork::wrap_plots(
  ...
  #boxplot
  ggplot(SR_Black_3, aes(x=factor(SRoutcome,
                                      levels = c("1","0"),
                                      labels = c("Type 1", "Type 2")),
                             y = pred_prob_m3)) +
    geom_violin(aes(fill = SRoutcome, alpha = 0.2)) +
...
####PDF
pdf("figures/Supp_Figure17.pdf", height = 20, width = 16)
...
dev.off()
ggsave("figures/Supp_Figure17.jpeg", model_display_item, height = 20, width = 16)
