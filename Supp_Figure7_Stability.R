...
stability_ci4 <- as_tibble(bootstrapped_estimations) %>%
  group_by(id) %>%
  summarise(q1 = quantile(.,probs = 0.025, na.rm = TRUE),
            q3 = quantile(.,probs = 0.975, na.rm = TRUE))


stability_plot4 <- original_predictions4 %>%
  as.data.frame() %>%
  rename("original_pred" = ".") %>%
  left_join(
    bootstrapped_estimations %>%
...
### Joint Stability PDF -----------------------------------------------------------
pdf("figures/Supp_Figure7.pdf", height = 20, width = 21)
...
dev.off()
ggsave("figures/Supp_Figure7.jpeg", stability_plots, height = 20, width = 21)
