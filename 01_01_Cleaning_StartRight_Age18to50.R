...
#load data-------------------------------------------------------------------------
load("~/PhD/StartRight_paper/T1DvsT2D_atDiagnosis_adults/data/SR_30_6_2025.RData")
#1) Filtering --------------------------------------------------------------------------
## A) For SRoutcome -------------------------------------------------------------------------------
#CREATE SR 18-50s
SR_SRout <- SR %>%
...
check <- SR %>%
  ...
  filter(!is.na(SRoutcome))
table(check$define)
table(check$robust_define)
define_breakdown_18_50 <- check %>%
  group_by(define) %>%
  summarise(
    n = n()
  )
write_xlsx(define_breakdown_18_50,"tables/define_breakdown_18_50.xlsx")
define_breakdown_18_50_robust <-check %>%
  group_by(robust_define) %>%
  summarise(
    n = n()
  )
write_xlsx(define_breakdown_18_50_robust,"tables/define_breakdown_18_50_robust.xlsx")
...
table(check$define)
table(check$robust_define)
define_breakdown_50plus <- check %>%
  group_by(define) %>%
  summarise(
    n = n()
  )
write_xlsx(define_breakdown_50plus,"tables/define_breakdown_50plus.xlsx")
define_breakdown_50plus_robust <-check %>%
  group_by(robust_define) %>%
  summarise(
    n = n()
  )
write_xlsx(define_breakdown_50plus_robust,"tables/define_breakdown_50plus_robust.xlsx")
## B) For robust_outcome -------------------------------------------------------------------------
SR %>%
  filter(AgeatDiagnosis > 50) %>%
  summarise(n = sum(!is.na(Study_ID) == TRUE))
