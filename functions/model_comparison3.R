...
model_comparisons <- function(
    varlist = NULL,
    varlist_cat = NULL,
    base_model_vars = NULL,
    base_model_vars_cat = NULL,
    dataset = NULL,
    outcome = NULL,
    saving_name = NULL,
    complete_case = TRUE,
  ...
  # Argument validation
  #if (is.null(varlist)) stop("'varlist' must be provided.")
  #if (is.null(varlist_cat)) stop("'varlist_cat' must be provided.")
  if (is.null(varlist) & is.null(varlist_cat)) stop("'varlist' or 'varlist_cat' must be provided.")
  if (is.null(base_model_vars)) stop("'base_model_vars' must be provided.")
  if (is.null(dataset)) stop("'dataset' must be provided.")
  if (is.null(outcome)) stop("'outcome' must be provided.")
  ...
  if (!is.null(varlist_cat) && length(varlist_cat) > 0) {
  for(var_cat in varlist_cat){
    # Ensure 'categories' is properly initialized
    if (!is.null(dataset_cc[[var_cat]])) {
      categories <- unique(dataset_cc[[var_cat]])
    } else {
      stop(paste("Variable", var_cat, "is not found in the dataset."))
    }
    # Loop over each group to add columns in the "x" and "y" alternating pattern
  ...
  #browser()
  if (nrow(uni_cat_coeffs) > 0) {
    model_comp_table <- full_join(model_comp_table, uni_cat_coeffs,
                                  by = intersect(names(model_comp_table),
                                                 names(uni_cat_coeffs)))
  } else {
    message("No categorical variables were processed. Returning results for continuous variables only.")
  }
  #model_comp_table <- full_join(model_comp_table, uni_cat_coeffs)
  ...
  if(is.null(saving_name)){
    ...
    if(plots == TRUE){
      ...
      dev.off()
        ...
        warning("'varlist_cat' is NULL or empty. No plots will be generated.")
      }
    }
  }
