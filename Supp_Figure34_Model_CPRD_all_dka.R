...
`1_ci`$prob_label
`0_ci`$prob_label
b_2to30 <- prop.test(x=0, n=24, conf.level = 0.95, correct = FALSE)
`2_ci` <- `2_ci` %>%
  add_row(prob4 = "0.2-0.3",
          prob_label = "0.2-0.3\n (n=24)",
          percent_row = 0,
          lcl_row = b_2to30$conf.int[1]*100,
          ucl_row = b_2to30$conf.int[2]*100)
`2_ci` <- `2_ci` %>%
  left_join(data %>%
...
plotA <- ggplot(`2_ci`,
  ...
  coord_cartesian(xlim = c(1, 5), ylim = c(-2,50),clip = "off") +
...
pdf("figures/Supp_Figure33.pdf", height = 16, width = 24)
...
print(joint_plot)
dev.off()
#ggsave("figures/Figure5_dka.png", joint_plot, height = 16, width = 24)
ggsave("figures/Supp_Figure33.jpeg", joint_plot, height = 16, width = 24)
